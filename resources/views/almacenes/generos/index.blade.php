@extends('layout') @section('content')
    @include('almacenes.generos.create')
    @include('almacenes.generos.edit')
@section('almacenes-active', 'active')
@section('genero-active', 'active')
<div class="row wrapper border-bottom white-bg page-heading">
    <div class="col-lg-10 col-md-10">
        <h2 style="text-transform:uppercase"><b>Listado de Generos del Producto terminado</b></h2>
        <ol class="breadcrumb">
    ...
    <div class="col-lg-2 col-md-2">
        <a data-toggle="modal" data-target="#modal_crear_genero" class="btn btn-block btn-w-m btn-primary m-t-md"
            href="#">
...
<script>
    ...
    function obtenerData($id) {
        ...
        data.each(function(value, index) {
            if (value.id == $id) {
                ...
                $('#genero_editar').val(value.nombre);
            }
        });
    ...
    @if ($errors->has('nombre'))
        $('#modal_editar_genero').modal({
            show: true
        });
    @endif

    function limpiarError() {
        $('#genero_editar').removeClass("is-invalid")
        $('#error-genero').text('')
    }

    $('#modal_editar_genero').on('hidden.bs.modal', function(e) {
        limpiarError()
    });

    //Old Modal Crear
    @if ($errors->has('descripcion_guardar'))
        $('#modal_crear_genero').modal({
            show: true
        });
    @endif

    function guardarError() {
        $('#genero_guardar').removeClass("is-invalid")
        $('#error-genero-guardar').text('')
    }

    $('#modal_crear_genero').on('hidden.bs.modal', function(e) {
        guardarError()
        $('#genero_guardar').val('')
    ...
    function eliminar(id) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                //Ruta Eliminar
                var url_eliminar = '{{ route('almacenes.genero.destroy', ':id') }}';
