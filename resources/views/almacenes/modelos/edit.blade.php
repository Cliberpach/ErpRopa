<div class="modal inmodal" id="modal_editar_modelo" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content animated bounceInRight">
            ...
            <form role="form" action="{{ route('almacenes.modelo.update') }}" method="POST" id="editar_modelo">
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control {{ $errors->has('modelo') ? ' is-invalid' : '' }}"
                            name="nombre" id="modelo_editar" value="{{ old('modelo') }}" onkeyup="return mayus(this)"
                            required>

                        @if ($errors->has('modelo'))
                            <span class="invalid-feedback" role="alert">
                                <strong id="error-modelo">{{ $errors->first('modelo') }}</strong>
                            </span>
                        @endif
                    </div>
                </div>
...
@push('scripts')
    <script>
        $(document).ready(function() {
            $("#modelo_editar").on("change", validarNombre);
        })
