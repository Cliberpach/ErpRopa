    ...
    <div class="modal-dialog">
        <div class="modal-content animated bounceInRight">
            ...
            <div class="modal-body">
                ...
                <form role="form" action="{{ route('almacenes.talla.store') }}" method="POST" id="crear_talla">
                    {{ csrf_field() }} {{ method_field('POST') }}

                    <div class="form-group">
                        <label class="required">Talla:</label>
                        <input type="text"
                            class="form-control {{ $errors->has('talla_guardar') ? ' is-invalid' : '' }}"
                            name="talla_guardar" id="talla_guardar" value="{{ old('talla_guardar') }}"
                            onkeyup="return mayus(this)" required>

                        @if ($errors->has('talla_guardar'))
                            <span class="invalid-feedback" role="alert">
                                <strong id="error-talla-guardar">{{ $errors->first('talla_guardar') }}</strong>
                            </span>
                        @endif
                    </div>
            </div>
...
@push('scripts')
    <script>
        $(document).ready(function() {
            $("#talla_guardar").on("change", validarNombre);
        })
