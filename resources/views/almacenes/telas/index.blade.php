@extends('layout') @section('content')
    @include('almacenes.telas.create')
    @include('almacenes.telas.edit')
@section('almacenes-active', 'active')
@section('tela-active', 'active')
<div class="row wrapper border-bottom white-bg page-heading">
    <div class="col-lg-10 col-md-10">
        <h2 style="text-transform:uppercase"><b>Listado de Telas del Producto terminado</b></h2>
        <ol class="breadcrumb">
    ...
    <div class="col-lg-2 col-md-2">
        <a data-toggle="modal" data-target="#modal_crear_tela" class="btn btn-block btn-w-m btn-primary m-t-md"
            href="#">
...
<script>
    ...
    function obtenerData($id) {
        ...
        data.each(function(value, index) {
            if (value.id == $id) {
                ...
                $('#tela_editar').val(value.nombre);
            }
        });

        $('#modal_editar_tela').modal('show');
    ...
    @if ($errors->has('nombre'))
        $('#modal_editar_tela').modal({
            show: true
        });
    @endif

    function limpiarError() {
        $('#tela_editar').removeClass("is-invalid")
        $('#error-tela').text('')
    }

    $('#modal_editar_tela').on('hidden.bs.modal', function(e) {
        limpiarError()
    });

    //Old Modal Crear
    @if ($errors->has('descripcion_guardar'))
        $('#modal_crear_tela').modal({
            show: true
        });
    @endif

    function guardarError() {
        $('#tela_guardar').removeClass("is-invalid")
        $('#error-tela-guardar').text('')
    }

    $('#modal_crear_tela').on('hidden.bs.modal', function(e) {
        guardarError()
        $('#tela_guardar').val('')
    ...
    function eliminar(id) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                //Ruta Eliminar
                var url_eliminar = '{{ route('almacenes.tela.destroy', ':id') }}';
